...
$(document).ready(function() {
    ...
    $('#preentrenoContainer img').on('click', function() {
        ...
        if (lastQuestion === null){
			sendResponseStatusGupoM(userId, option, idQuestion, null, null, null);
    ...
    $('#testCarasOkContainer img').on('click', function() {
        ...
        sendResponseStatusGupoM(userId, option, idQuestion, statement, "OK", estimuloAbs);
    ...
    $('#testCarasKoContainer img').on('click', function() {
        ...
        var estimuloAbs = $(this).data('estabstracto')
        console.log('estimulo abs :: '+ estimuloAbs)
        sendResponseStatusGupoM(userId, option, idQuestion, statement, "KO", estimuloAbs);
    ...
    $('#testEquivalencia img').on('click', function() {
    	...
    	sendResponseStatusGupoM(userId, option, idQuestion, null, statusMano, estimuloAbs);  
    ...
    $('#testEquivalencia2 img').on('click', function() {
        ...
        lastQuestion = takeTestEquivalencia();      
         if (lastQuestion === null){			
		 	takeGroupQuestion();
			 $("#grupoM").removeClass("inactive").addClass("active");
    ...
    $('#grupoM img').on('click', function(){
		var option = $(this).data('option');
        var idQuestion = $(this).data('idquestion');
        var statusMano = $(this).data('tipomano');
        var srcValue = $(this).attr('src');
        sendResponseStatusGupoM(userId, option, idQuestion, null, statusMano, null);
        if (option === "succes"){
			goupSelectionStatus = true;
		}else{
			goupSelectionStatus = false;
		}
        lastQuestionGroup2 = takeGroupQuestion2(goupSelectionStatus, srcValue, false);
        if (lastQuestionGroup2 !== null){
			processSelctImgGoupsAlterno(option, "grupoM")
		}
	})
	$('#grupoM2 img').on('click', function(){
		var option = $(this).data('option');
        var idQuestion = $(this).data('idquestion');
        sendResponseStatusGupoM(userId, option, idQuestion, null, null, null);
        lastQuestionGroup = takeGroupQuestion(goupSelectionStatus);
        processSelctImgGoupsAlterno(option, "grupoM2")
	})
});
...
function processSelctImgGoupsAlterno(option, questionContainer) {
	$("#" + questionContainer).removeClass("active").addClass("inactive");
    if (option === 'succes') {
		$("#correctAudio")[0].play();
   ...
   setTimeout(function() {
        ...
        $("#" + questionContainer).removeClass("active").addClass("inactive"); 	
        if (questionContainer === 'grupoM'){
			$("#grupoM2").removeClass("inactive").addClass("active");
		}if (questionContainer === 'grupoM2'){
			$("#grupoM").removeClass("inactive").addClass("active");
		}  
    }, 500);
}
...
function takeOutPreentrenoQuestion(questionIds) {
    ...
    if (questionSetIsComplete(20, "preentrenoContainer", countSets)){
        question = getQuestionRandom(questionIds, 20, countPreentrenoQuestions);
...
function takeTestEquivalencia(){
    ...
    if (questionTestEquivIsComplete(28, "testEquivalencia", countSets)){      		
	    var question  = getQuestionRandom(questionIdsTestEquivalencia, 14, countQuestionsTestEquivalencia);
...
function takeGroupQuestion(goupSelectionStatus){
	var questionIds = ["grupoOk","grupoKo"];
	var countSets = 0;
    questionIds.forEach(function (id) {
        $("#" + id).removeClass("active").addClass("inactive");
        countSets = countSets + countQuestionsGroup[id];
    });
    if (groupQuestionSetIsCompleteGroup(28, "grupoM", countSets)){
        ...
        takeGroupQuestion2(goupSelectionStatus, true);
        return null;
    }
}
function takeGroupQuestion2(goupSelectionStatus, srcValue , finalQuestion){	
	var countSets = 0;
	var questionId = ['grupoKo2','grupoOk2']
    questionId.forEach(function (id) {
        $("#" + id).removeClass("active").addClass("inactive");
    });
    ...
    if (finalQuestion == false){
        ...
        if (goupSelectionStatus === true){
			$("#grupoOk2").removeClass("inactive").addClass("active");
			$(".headerImgOk").attr('src', srcValue);
			$("#" + question).removeClass("inactive").addClass("active");		 
		}if (goupSelectionStatus === false){
			$("#grupoKo2").removeClass("inactive").addClass("active");
			$("#" + question + '-KO').removeClass("inactive").addClass("active");
			$(".headerImgkO").attr('src', srcValue);
		}		
...
function sendResponseStatusGupoM(userId , option, idQuestion, statement, mano_ok, estimuloAbs) {    
     let result = false;
